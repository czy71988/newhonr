...
<div class="zhuce2">
  ...
  <el-form class="formOne form" :model="formOne" status-icon :rules="rules" v-if="value === 1" ref="formOne" @submit.native.prevent>
    <el-form-item prop="username">
      <el-input v-model="formOne.username" autocomplete="off" placeholder="请输入姓名"></el-input>
    </el-form-item>
    <el-form-item prop="province">
      <el-select v-model="formOne.province" placeholder="省" class="aj6" @change="handleProvinceChange">
    ...
    <el-form-item prop="platform">
      <el-select v-model="formOne.platform" multiple placeholder="请选择店铺所属平台" class="aj6">
    ...
    <el-form-item prop="shopname">
      <el-input v-model="formOne.shopname" autocomplete="off" placeholder="请输入店铺名称"></el-input>
    </el-form-item>
    <button class="zhuce-btn" @click.stop="handleStepNext('formOne')">下一步</button>
  </el-form>
  <!-- 第二步 formTwo -->
  <el-form class="formTwo form" :rules="rules" :model="formTwo" v-else-if="value === 2" @submit.native.prevent ref="formTwo">
    ...
    <div class="business-license-ctn">
      <el-upload
        ...
        :action="actions.updateBusinessLicense + '&token=' + token"
        ...
        :on-error="(error, file) => handleUploadPicError(error, 'business')">
         <div class="upload-animation-box"
          ...
          <img v-if="formTwo.businessimg" :src="formTwo.businessimg" class="business-license">
      ...
      <div class="id-card-box">
        <p class="title">正面</p>
         <div class="img-wrap upload-animation-box"
            ...
            element-loading-text="拼命上传中">
          <img :src="formTwo.cardFrontimg || frontIddCardPlaceholderPic" class="id-card-img" alt="">
        </div>
        <el-upload
          :action="actions.updateFrontIDCard + '&token=' + token"
          list-type="text"
          accept="image/*"
    ...
    <div class="button-group">
      ...
      <button class="btn--22 go-next-btn mb-btn" @click="handleStepNext('formTwo')">下一步</button>
    </div>
  </el-form>
  <!-- 第三步 formThree -->
  <el-form class="formThree form" :model="formThree" status-icon :rules="rules" v-else-if="value === 3" ref="formThree" @submit.native.prevent>
    <el-form-item>
      <el-input v-model="formThree.wechat" placeholder="请输入微信账号"></el-input>
    </el-form-item>
    <el-form-item>
      <el-input v-model="formThree.dingding" placeholder="请输入钉钉账号"></el-input>
    </el-form-item>
    <el-form-item>
      <el-input v-model="formThree.wangwang" placeholder="请输入旺旺账号"></el-input>
    </el-form-item>
    <!-- <el-form-item label="">
      <el-checkbox-group v-model="formTwo.type4">
        <el-checkbox label="请仔细阅读此协议" name="1"></el-checkbox>
      </el-checkbox-group>
    </el-form-item> -->
    <div class="button-group">
      <button class="btn--11 mb-btn" @click="handleStepBack">返回上一步</button>
      <button class="btn--22 mb-btn" @click="handleSubmit('formThree')">提交修改</button>
    </div>
  </el-form>
</div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      areaMap: null, // 地区
      platformshangjia,
      contentCategoryStoreData, // 商家内容分类
      frontIdCardPlaceholderPic, // 身份证正面占位图
      backIdCardPlaceholderPic, // 身份证反面占位图
      holdIdCardPlaceholderPic,
      rules: {
        epname: [
          { required: true, message: '请填写企业名称', trigger: ['blur', 'change'] }
        ],
        creditcoding: [
          { required: true, message: '请填写企业统一社会信用代码', trigger: ['blur', 'change'] }
        ],
        username: [
  ...
  created () {
    ...
    const formThree = this.formThree
    ...
    Object.keys(formThree).forEach(item => {
      formThree[item] = infData[item]
    })
  ...
  methods: {
      ...
      // 正确可下一步
        if (valid) {
          if (formName === 'formTwo') {
            ...
            if (formName === 'formOne') window.scrollTo(0, 0)
            this.$emit('input', this.value + 1)
          }
        }
      })
    ...
    handleUploadPicSuccess (file, operatorType) {
      if (operatorType === 'business') {
        this.formTwo.businessimg = URL.createObjectURL(file.raw)
